...
From the histogram, the distribution looks relatively normal, with a slightly positive, right-skew. The median value is less than the corresponding mean value, because there are a greater number of outliers within the older population, slightly affecting the mean value.
...
```{r}
nh_black <- t.test (maj_race == "nh_black")
hs_latino <- t.test (maj_race == "hs_latino")
no_majority <- t.test (maj_race == "no_majority")
nh_white <- t.test (maj_race == "nh_white")
nh_asian <- t.test (maj_race == "nh_asian")
```
